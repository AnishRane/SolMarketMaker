...
export class MarketMaker {
    ...
    async determineTradeNecessity(jupiterClient: JupiterClient, pair: any, token0Balance: Decimal, token1Balance: Decimal) {
        ...
        let brkAmountToTrade = new Decimal(0);
        ...
        if (token0Value.gt(targetValuePerToken)) {
            ...
            brkAmountToTrade = valueDiff.div(token1Price);
            tradeNeeded = true;
        }

        const minimumTradeAmount = new Decimal(0.01);
        if (solAmountToTrade.lt(minimumTradeAmount) && brkAmountToTrade.lt(minimumTradeAmount)) {
            tradeNeeded = false;
        }

        return { tradeNeeded, solAmountToTrade, brkAmountToTrade };
    }
